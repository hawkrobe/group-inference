...
gendat = zip_gendat(gendat_self_norm_noauto_grp)
# pooling
pool = multiprocessing.Pool()
poolout = pool.map(tomtom_refit,gendat)
modrec_seeds_self_norm_noauto_grp = [i[0] for i in poolout]
modrec_maps_self_norm_noauto_grp = [i[1] for i in poolout]
modrec_logprobs_self_norm_noauto_grp = [i[2] for i in poolout]

# save
with open('modrec_self_norm_noauto_grp.pkl','wb') as f:
    pickle.dump([modrec_seeds_self_norm_noauto_grp,modrec_maps_self_norm_noauto_grp,modrec_logprobs_self_norm_noauto_grp],f)
