...
gendat = zip_gendat(gendat_self_raw_all_dim)
# pooling
pool = multiprocessing.Pool()
poolout = pool.map(tomtom_refit,gendat)
modrec_seeds_self_raw_all_dim = [i[0] for i in poolout]
modrec_maps_self_raw_all_dim = [i[1] for i in poolout]
modrec_logprobs_self_raw_all_dim = [i[2] for i in poolout]

# save
with open('modrec_self_raw_all_dim.pkl','wb') as f:
    pickle.dump([modrec_seeds_self_raw_all_dim,modrec_maps_self_raw_all_dim,modrec_logprobs_self_raw_all_dim],f)
